...
export default function ArticleGallery({ article }) {
  ...
  const images = article.gallery && article.gallery.length >= 3
    ? article.gallery
    : [{ url: article.featuredImage, caption: article.title }];

  return (
    <section className="exp-gallery-section section" aria-label={`${article.title} Photo Gallery`}>
