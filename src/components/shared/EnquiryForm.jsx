...
export default function EnquiryForm({
  ...
  minLengthErrorText = (n) => `Please enter at least ${n} characters.`,
  formId,
  onSubmit,
}) {
  ...
  const handleSubmit = async (e) => {
    ...
    fields.forEach((f) => {
      ...
      if (f.required && !val) {
        nextErrors[f.name] = requiredErrorText;
      } else if (f.type === 'email' && val && !/\S+@\S+\.\S+/.test(val)) {
        nextErrors[f.name] = invalidEmailErrorText;
      } else if (f.minLength && val && val.length < f.minLength) {
        nextErrors[f.name] = minLengthErrorText(f.minLength);
      }
    });
